...
const SAMPLE_LOGS: Log[] = [
  { id: "1", timestamp: "2024-11-08T14:32:45Z", level: "info",    service: "api-gateway",     message: "Request processed successfully",       duration: "245ms", status: "200",     tags: ["api", "success"] },
  { id: "2", timestamp: "2024-11-08T14:32:42Z", level: "warning", service: "cache-service",   message: "Cache miss ratio exceeds threshold",   duration: "1.2s",  status: "warning", tags: ["cache", "performance"] },
  { id: "3", timestamp: "2024-11-08T14:32:40Z", level: "error",   service: "database",        message: "Connection timeout to replica",         duration: "5.1s",  status: "503",     tags: ["db", "error"] },
  { id: "4", timestamp: "2024-11-08T14:32:38Z", level: "info",    service: "auth-service",    message: "User session created",                 duration: "156ms", status: "201",     tags: ["auth", "session"] },
  { id: "5", timestamp: "2024-11-08T14:32:35Z", level: "info",    service: "api-gateway",     message: "Webhook delivered",                    duration: "432ms", status: "200",     tags: ["webhook", "integration"] },
  { id: "6", timestamp: "2024-11-08T14:32:32Z", level: "error",   service: "payment-service", message: "Payment gateway unavailable",          duration: "2.3s",  status: "502",     tags: ["payment", "error"] },
  { id: "7", timestamp: "2024-11-08T14:32:30Z", level: "info",    service: "search-service",  message: "Index updated",                        duration: "876ms", status: "200",     tags: ["search", "index"] },
  { id: "8", timestamp: "2024-11-08T14:32:28Z", level: "warning", service: "api-gateway",     message: "Rate limit approaching",               duration: "145ms", status: "429",     tags: ["rate-limit", "warning"] },
];
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          <motion.div
            ...
            className="overflow-hidden border-t border-border bg-white/[0.02]"
          >
            <div className="space-y-4 p-4">
              <div>
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Message</p>
                <p className="rounded bg-white/[0.03] p-3 font-mono text-sm text-foreground">{log.message}</p>
              </div>
...
function FilterPanel({ filters, onChange, logs }: { filters: Filters; onChange: (f: Filters) => void; logs: Log[] }) {
  ...
  const toggleFilter = (category: keyof Filters, value: string) => {
    ...
    onChange({ ...filters, [category]: current.includes(value) ? current.filter((e) => e !== value) : [...current, value] });
  };
  ...
  return (
    ...
    >
      ...
      {(["level", "service", "status"] as (keyof Filters)[]).map((category) => {
        ...
        return (
          <div key={category} className="space-y-3">
            ...
            <div className="space-y-2">
              {options.map((opt) => {
                const selected = filters[category].includes(opt);
                return (
                  <motion.button
                    key={opt} type="button" whileHover={{ x: 2 }}
                    ...
                    className={`flex w-full items-center justify-between gap-2 border rounded-md px-3 py-2 text-sm transition-colors ${
                      selected
                        ...
                        : "border-border text-muted-foreground hover:border-primary/40 hover:bg-white/[0.04]"
                    }`}
                  >
...
export function InteractiveLogsTable() {
  ...
  const filteredLogs = useMemo(() => {
    return SAMPLE_LOGS.filter((log) => {
      ...
      return (
        (log.message.toLowerCase().includes(q) || log.service.toLowerCase().includes(q)) &&
        ...
        <div className="border-b border-border bg-card p-6">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{filteredLogs.length} of {SAMPLE_LOGS.length} logs</p>
            </div>
